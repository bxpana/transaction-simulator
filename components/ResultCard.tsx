...
export function ResultCard({ 
  ...
  const formatBlockNumber = (value?: bigint) => value !== undefined ? `#${value.toString()}` : "--";
          ...
          {(showMockData || result?.status === "success" || isLiveRunning) ? (
            <>
              <div className="mb-3">
                ...
                {!showMockData && inclusion && (
                  <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-zinc-300 font-mono bg-zinc-800/50 border border-zinc-700/50 rounded px-3 py-2">
                    <div className="flex justify-between">
                      <span className="text-zinc-400">Send block</span>
                      <span className="text-white">{formatBlockNumber(inclusion.sendBlockNumber)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-400">Receipt block</span>
                      <span className="text-white">{formatBlockNumber(inclusion.receiptBlockNumber)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-400">Block delta</span>
                      <span className="text-white">
                        {inclusion.blockDelta !== undefined ? `${inclusion.blockDelta}` : "--"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-zinc-400">Inclusion wait</span>
                      <span className="text-white">
                        {inclusion.inclusionLatencyMs !== undefined ? `${inclusion.inclusionLatencyMs}ms` : "--"}
                      </span>
                    </div>
                  </div>
                )}
